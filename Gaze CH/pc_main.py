...
# -----------------------------------
def find_com_port(): #only called once before opening serial
...
# 3. Open COM
# -----------------------------------
ser = serial.Serial(PORT, 9600, timeout=1) #once befrore loop

# -----------------------------------
# 4. Interpret gaze into directions
# -----------------------------------

keyboard = Controller()  # Create keyboard controller

def get_direction_VK(direction):
    """
    Simulate arrow key press based on direction.
    
    Args:
        direction: String - "UP", "DOWN", "LEFT", "RIGHT", or "CENTER"
    """
    if direction == "directon_center":
        # Do nothing or release all keys
        return
    
    elif direction == "directon_right":
        keyboard.press(Key.right)
        keyboard.release(Key.right)
        print("➡️ Right Arrow")
    
    elif direction == "directon_left":
        keyboard.press(Key.left)
        keyboard.release(Key.left)
        print("⬅️ Left Arrow")
    
    elif direction == "directon_up":
        keyboard.press(Key.up)
        keyboard.release(Key.up)
        print("⬆️ Up Arrow")
    
    elif direction == "directon_down":
...
def scroll_mode(scroll_command):
    if scroll_command == "scroll_up":
        pynput.mouse.Controller().scroll(0, 2)  # Scroll up
        print("Scrolled Up")
    elif scroll_command == "scroll_down":
        pynput.mouse.Controller().scroll(0, -2)  # Scroll down
        print("Scrolled Down")
    else:
        print("No Scroll Action")

def vk_mode(vk_command):
...
while True:
    ...
    #line = "scroll_up" # For testing without serial input
    line = "vk_open"
    if not line:
        continue
    ...
    if line.startswith("Gaze:"):
        try:
            ...
            direction = get_direction_XY(x, y)
            print("➡️ Direction:", direction)

        except Exception as e:
            print("⚠️ Parse error:", e)
    
    elif line.startswith("directon_"):
        get_direction_VK(line)

    elif line.startswith("click_"):
        get_and_map_clicks(line)
