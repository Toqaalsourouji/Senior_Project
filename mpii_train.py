...
def parse_args():
    ...
    parser.add_argument("--pin-memory", action="store_true", help="Use pinned memory for faster GPU transfer.")
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        for i in range(torch.cuda.device_count()):
            ...
            logging.info(f"  Compute Capability: {torch.cuda.get_device_properties(i).major}.{torch.cuda.get_device_properties(i).minor}")
    else:
        logging.warning("⚠️  CUDA NOT AVAILABLE - Will train on CPU (very slow!)")
    
    logging.info("="*60)

    # Setup device with GPU info
    device = setup_device(params.gpu)
    ...
    if torch.cuda.is_available():
        ...
        logging.info("TF32 enabled for faster training")
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        # Create data loaders for the subsets with pin_memory for faster GPU transfer
        train_loader = torch.utils.data.DataLoader(
            train_subset, 
            batch_size=params.batch_size, 
            shuffle=True,
            num_workers=params.num_workers,
            pin_memory=params.pin_memory if torch.cuda.is_available() else False
        )
        ...
        avg_error = evaluate(params, model, val_loader, idx_tensor, device)
