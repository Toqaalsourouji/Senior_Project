...
# utils/helpers.py  (add near the top)
from torchvision import transforms
from torch.utils.data import DataLoader


# add this import:
from dataset.datasets.mpiigaze_raw import MPIIGazeRaw
...
def get_dataloader(params, mode="train"):
    ...
    # Default transform (used by gaze360 below)
    transform = transforms.Compose([
        transforms.Resize(448),  # single int -> resize smaller edge, keep aspect
        transforms.ToTensor(),
    ...
    if params.dataset == "gaze360":
        dataset = Gaze360(params.data, transform,
                          angle=params.angle, binwidth=params.binwidth, mode=mode)
        ...
        # Typical 224x224 for ResNet/MobileNet family
        tfm = transforms.Compose([
            transforms.Resize((224, 224)),  # explicit HxW
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                 std=[0.229, 0.224, 0.225]),
        ])
        ...
        # Build kwargs so we only pass prefetch_factor when num_workers>0
        loader_kwargs = dict(
            dataset=dataset,
            batch_size=params.batch_size,
            shuffle=(mode == "train"),
        ...
        if params.num_workers and params.num_workers > 0:
        ...
        raise ValueError("Supported datasets are `gaze360` and `mpiigaze`")
