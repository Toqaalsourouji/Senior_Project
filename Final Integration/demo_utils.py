...
def draw_gaze(image_in, eye_pos, pitchyaw, length=15.0, thickness=2, color=(0, 0, 255),
              reference_pitchyaw=None):
    """Draw gaze angle on given image with a given eye positions.
    
    Args:
        image_in: Input image
        eye_pos: (x, y) coordinates of eye center
        pitchyaw: (pitch, yaw) angles in radians
        length: Length of the arrow in pixels
        thickness: Thickness of the arrow line
        color: BGR color tuple
        reference_pitchyaw: Reference (pitch, yaw) for calibration. If None, uses raw values.
    """
    ...
    if reference_pitchyaw is not None:
        adjusted_pitch = pitchyaw[0] - reference_pitchyaw[0]
        adjusted_yaw = pitchyaw[1] - reference_pitchyaw[1]
    else:
        adjusted_pitch = pitchyaw[0]
        adjusted_yaw = pitchyaw[1]
    
    # Calculate arrow endpoint
    dx = -length * np.sin(adjusted_yaw) * np.cos(adjusted_pitch)
    dy = -length * np.sin(adjusted_pitch)
