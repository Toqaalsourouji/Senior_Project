...
class OneEuroFilter:
    ...
    def __init__(self, t0=-1.0, x0=0.0, dx0=0.0, min_cutoff=1.0, beta=0.01,
                 d_cutoff=1.0):
...
class GazeSmoother:
    ...
    def __call__(self, pitchyaw, **kwargs):
        return self.pitch_smoother(pitchyaw[0], **kwargs), self.yaw_smoother(pitchyaw[1], **kwargs)
...
class LandmarkSmoother:
    ...
    def __call__(self, landmark, **kwargs):
        ...
        ret = [[self.filters[i][0](landmark[i][0], **kwargs), self.filters[i][1](landmark[i][1], **kwargs)] for i in range(self.pt_num)]
