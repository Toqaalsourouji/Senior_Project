...
class GazeCalibrationSystem:
    ...
    def __init__(self, picam2, samples_per_position: int = 15,
                 sample_collection_time: float = 5.5,
                 output_file: str = "gaze_direction_calibration.json", serial_obj: serial.Serial = None, FDS = None , GDS = None, LD = None): # For Toqa LD = LandMark Detection
        """
        ...
        self.done = False
        self.finished_cal = False
        
        
        self.picam2 = picam2
        
        self.screen_width = 1920 #hardcoded, yet we can create a listner here that gets the actual sizes from the pc on the pc script 
        self.screen_height = 1080 #same as the width
        ...
        self.gaze_smoother = None
        
        #Storage for calibration data
        self.directions: Dict[str, GazeDirection] = {}
        
        #Initalize gaze tracking components LAST
        self.face_detection_session = FDS 
        self.landmark_detection_session = LD
        self.gaze_estimation_session = GDS
   ...
   #     try:
    #        # Load ONNX models
     #       #self.face_detection_session = onnxruntime.InferenceSession("./models/face_detection.onnx")
      #      self.face_detection_session = FDS 
            #self.landmark_detection_session = onnxruntime.InferenceSession("./models/landmark_detection.onnx") #remove
       #     self.landmark_detection_session =  
            #self.gaze_estimation_session = onnxruntime.InferenceSession("./models/gaze_estimation.onnx")
            
        #    time.sleep(0.5)  # give sensor time to warm up
         #   print("PiCamera2 initialized for calibration")
    ...
    def _get_current_gaze(self) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            if not hasattr(self, '_gaze_detector'):
                #make sure tracking is initialized first 
                if not hasattr(self, 'face_detection_session'):
    ...
    def collect_samples_for_position(self, position_name: str) -> Tuple[List[float], List[float]]:
        ...
        while len(pitch_samples) < self.samples_per_position:
            ...
            time.sleep(0.01) #to prevent CPU spinning
...
Example usage:
    ...
    calibrator = GazeCalibrationSystem(
        None,
        samples_per_position=15, # was args.samples
        sample_collection_time=5.5, # was args.time
        output_file='Calibration.json' # was args.output
    )
